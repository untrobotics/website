...
/**
 * URW-52: validate a `returnto` redirect target. Only a same-origin, root-
 * relative path is allowed — a single leading slash followed by a non-slash
 * (blocks "//evil.com" and "/\evil.com" protocol-relative tricks), no scheme,
 * no backslash, no CR/LF (header injection), and never a path under /auth (which
...
/**
 * URW-52: build a "?returnto=<url-encoded>" suffix for a login/join link.
 * Defaults to the current request URI; returns "" when the target isn't a valid
 * returnto, so the link degrades to a plain /auth/login.
 */
