...
// Combined "what we build" + "competitions" showcase (a single page so the nav
// stays lean). Photos live under /images/content/. No image is shared between
// the two sections. Each card: hero image, tag, blurb, optional gallery/video.
$sections = array(
    array(
        'id' => 'projects',
        'heading' => 'What We Build',
        'sub' => 'From a self-driving sofa to an autonomous rover, here&rsquo;s what our members make &mdash; across every discipline and skill level.',
        'items' => array(
            array(
                'title' => 'Scrapp-E',
                'tag' => 'Flagship Robot',
    ...
    array(
        ...
        'sub' => 'We put our skills to the test against schools across the country &mdash; and host our own.',
        'items' => array(
            array(
                'title' => 'High-Power Rocketry',
                'tag' => 'NASA Student Launch',
...
?>
<style>
    .proj-hero { padding: 60px 0 10px; text-align: center; }
    .proj-hero h1 { margin-bottom: 10px; }
    .proj-hero p { max-width: 720px; margin: 0 auto; color: #555; font-size: 17px; line-height: 1.6; }
    .proj-section-head { max-width: 1080px; margin: 30px auto 0; padding: 0 15px; text-align: center; }
    .proj-section-head h2 { margin-bottom: 8px; }
    .proj-section-head p { color: #666; max-width: 680px; margin: 0 auto; }
    .proj-list { max-width: 1080px; margin: 0 auto; padding: 24px 15px 30px; }
    .proj { display: flex; gap: 34px; align-items: flex-start; margin-bottom: 56px; }
    .proj.reverse { flex-direction: row-reverse; }
    .proj-media { flex: 0 0 46%; max-width: 46%; }
    .proj-media .main { width: 100%; height: 380px; object-fit: cover; border-radius: 10px; display: block; box-shadow: 0 6px 24px rgba(0,0,0,.12); cursor: zoom-in; }
    ...
    @media (max-width: 767px) {
        .proj, .proj.reverse { flex-direction: column; gap: 16px; }
        .proj-media, .proj.reverse .proj-media { max-width: 100%; flex-basis: auto; }
    }
</style>
<main class="page-content">
    <section class="proj-hero">
        <div class="shell">
            <h1>What We Do</h1>
            <p>UNT Robotics is a &ldquo;design, build, test, and fly&rdquo; organization. Here&rsquo;s some of what our members build &mdash; and the competitions we take it to.</p>
        </div>
    </section>
    <?php $i = 0; foreach ($sections as $section): ?>
        <div class="proj-section-head" id="<?php echo htmlspecialchars($section['id']); ?>">
            <h2><?php echo $section['heading']; ?></h2>
            <p><?php echo $section['sub']; ?></p>
        </div>
        <div class="proj-list">
            <?php foreach ($section['items'] as $p): $reverse = ($i % 2 === 1); $i++; ?>
                <div class="proj<?php echo $reverse ? ' reverse' : ''; ?>">
                    ...
                    <div class="proj-body">
                        <span class="proj-tag"><?php echo $p['tag']; ?></span>
...
<script>
(function () {
    // Clicking any project image (main or thumbnail) opens it full-size. No swapping.
